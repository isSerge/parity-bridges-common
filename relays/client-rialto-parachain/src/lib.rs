...
impl Chain for RialtoParachain {
	...
	const BEST_FINALIZED_HEADER_ID_METHOD: &'static str = bp_rialto_parachain::BEST_FINALIZED_RIALTO_PARACHAIN_HEADER_METHOD;
	const AVERAGE_BLOCK_INTERVAL: Duration = Duration::from_secs(5);
	const STORAGE_PROOF_OVERHEAD: u32 = bp_rialto_parachain::EXTRA_STORAGE_PROOF_SIZE;
	const MAXIMAL_ENCODED_ACCOUNT_ID_SIZE: u32 = bp_rialto_parachain::MAXIMAL_ENCODED_ACCOUNT_ID_SIZE;
...
impl ChainWithBalances for RialtoParachain {
	fn account_info_storage_key(account_id: &Self::AccountId) -> StorageKey {
		...
		StorageKey(frame_system::Account::<rialto_parachain_runtime::Runtime>::storage_map_final_key(
			account_id,
		))
	}
}
...
impl ChainWithMessages for RialtoParachain {
	...
	const TO_CHAIN_LATEST_GENERATED_NONCE_METHOD: &'static str =
		bp_rialto_parachain::TO_RIALTO_PARACHAIN_LATEST_GENERATED_NONCE_METHOD;
	const TO_CHAIN_LATEST_RECEIVED_NONCE_METHOD: &'static str =
		bp_rialto_parachain::TO_RIALTO_PARACHAIN_LATEST_RECEIVED_NONCE_METHOD;
	const FROM_CHAIN_LATEST_RECEIVED_NONCE_METHOD: &'static str =
		bp_rialto_parachain::FROM_RIALTO_PARACHAIN_LATEST_RECEIVED_NONCE_METHOD;
	const FROM_CHAIN_LATEST_CONFIRMED_NONCE_METHOD: &'static str =
		bp_rialto_parachain::FROM_RIALTO_PARACHAIN_LATEST_CONFIRMED_NONCE_METHOD;
	const FROM_CHAIN_UNREWARDED_RELAYERS_STATE: &'static str =
		bp_rialto_parachain::FROM_RIALTO_PARACHAIN_UNREWARDED_RELAYERS_STATE;
...
impl TransactionSignScheme for RialtoParachain {
	...
	fn sign_transaction(
		...
		let raw_payload = SignedPayload::from_raw(
			...
			(
				...
				frame_system::CheckEra::<rialto_parachain_runtime::Runtime>::from(era.frame_era()),
				...
				pallet_transaction_payment::ChargeTransactionPayment::<rialto_parachain_runtime::Runtime>::from(unsigned.tip),
			),
			(
				rialto_parachain_runtime::VERSION.spec_version,
				genesis_hash,
		...
		rialto_parachain_runtime::UncheckedExtrinsic::new_signed(
			call,
	...
	fn is_signed_by(signer: &Self::AccountKeyPair, tx: &Self::SignedTransaction) -> bool {
		tx.signature
			.as_ref()
			.map(|(address, _, _)| *address == rialto_parachain_runtime::Address::Id(signer.public().into()))
			.unwrap_or(false)
	}
